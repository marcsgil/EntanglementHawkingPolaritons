...
function plot_window_pair(saving_dir, n, pair, steady_state, rs, param; xlims=nothing, ylims=nothing, savefig=false)
    with_theme(theme_latexfonts()) do
        ...
        weights1 = get_window_weights(pair.first)
        weights2 = get_window_weights(pair.second)
        ...
        lines!(ax, rs[first_idx1:(first_idx1+N1-1)], weights1; linewidth, label=L"k \text{ Window}", linestyle=:dash)
        lines!(ax, rs[first_idx2:(first_idx2+N2-1)], weights2; linewidth, label=L"k^\prime \text{ Window}", linestyle=:dot)
